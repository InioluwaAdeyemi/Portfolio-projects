...
#load original .csv files, a years worth of data from June 2021 to May 2022
...
cyclistic_df <- rbind (aug08_df, sep09_df, oct10_df, nov11_df, dec12_df, 
                       jan01_df, feb02_df, mar03_df, apr04_df, may05_df, 
                       jun06_df, jul07_df)

#remove individual month data frames to clear up space in the environment 
remove(aug08_df, sep09_df, oct10_df, nov11_df, dec12_df, 
        jan01_df, feb02_df, mar03_df, apr04_df, may05_df, 
        jun06_df, jul07_df)
...
#type of bikes: total rides by member type 
cyclistic_date %>%
  group_by(member_casual, rideable_type) %>% 
  count(rideable_type)

#type of bikes:total rides 
cyclistic_date %>%
  group_by(rideable_type) %>% 
  count(rideable_type)

#Hour: total rides by member type 
cyclistic_date %>%
  group_by(member_casual) %>% 
  count(hour) %>% 
